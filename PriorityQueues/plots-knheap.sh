...
#set logscale x
set xlabel "log(n)"
set ylabel "L3 Total Cache Hits"
#set yrange [0:50]
set output "priority_queue_cache_hits_level_2.eps"

plot "${FILE1}" using  (log(\$1)/log(2)):6 title "Kn heap" with linespoints, \
     "${FILE2}"  using (log(\$1)/log(2)):6 title "Kn heap adapted" with linespoints
GPLOT