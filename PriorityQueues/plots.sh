...
set format y "%.0s*10^%T"
#set yrange  [0.00000000:0.00000080]
set output "$PLOTOUTPUT/priority_queue_running_time_with_fitting.eps"

plot "$DATADIR/simple_heap_test.data" using (log(\$1)/log(2)):2 title "Binary heap",\
     "$DATADIR/sanders_heap4_test.data" using (log(\$1)/log(2)):2 title "Bottom up aligned 4-ary heap",\
     "$DATADIR/sanders_knheap_test.data" using (log(\$1)/log(2)):2 title "Kn heap", \
     "$DATADIR/simple_heap_test.data" using (log(\$1)/log(2)):(0.625*10**(-8)*log(\$1)/log(2)-0.5*10**(-9)) title "0.625*10^{-8}*log(n)", \
     "$DATADIR/simple_heap_test.data" using (log(\$1)/log(2)):(1.15*10**(-8)*log(\$1)/log(2)-80*10**(-9)) title "1.25*10^{-8}*log(n)", \
     "$DATADIR/sanders_knheap_test.data" using (log(\$1)/log(2)):(0.25*10**(-8)*log(\$1)/log(2)+30*10**(-9)) title "0.25*10^{-8}*log(n)"
