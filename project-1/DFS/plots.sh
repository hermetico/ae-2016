PLOTOUTPUT="plots - " $(date '+%d/%m/%Y %H:%M:%S')
DATADIR=$1
...
plot    "$DATADIR/dfs_array_recursive_continuous_data" using using (log(\$1)/log(2)):(\$2) title "100% finding propability", \
...
plot
    "$DATADIR/dfs_array_recursive_continuous_data" using (log(\$1)/log(2)):(\$3/(log(\$1)/log(2))) title "100% finding propability", \
...
plot
    "$DATADIR/dfs_array_recursive_continuous_data" using (log(\$1)/log(2)):4 title "L1 Cache misses", \
